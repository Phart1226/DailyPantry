...
//        navigationController?.navigationBar.barTintColor = UIColor(red: 55/255, green: 120/255, blue: 250/255, alpha: 1)
    ...
    func getPantryItems(){
        ...
        if (currentPantryItems["Meat"]?.names.count)! == 0{
            for item in pantryItems{
                currentPantryItems[(item.value(forKey: "category") as! String)]?.names.append((item.value(forKey: "name") as! String))
                currentPantryItems[(item.value(forKey: "category") as! String)]?.qtys.append((item.value(forKey: "qty") as! Int))
                
            }
        }else{
            for item in pantryItems{
                let spot = currentPantryItems[(item.value(forKey: "category") as! String)]?.names.firstIndex(of: (item.value(forKey: "name") as! String))
                currentPantryItems[(item.value(forKey: "category") as! String)]?.qtys[spot!] = (item.value(forKey: "qty") as! Int)
