...
class AddToPantryViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource  {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        if pickerView.tag == 1 {
            ...
            qtyVal = selectedItem.value(forKey: "amountAvailable") as! Int
    ...
    func getItems(){
        for item in groceryItems{
            if (item.value(forKey: "catagory") as! String) == catSelected{
    ...
    @IBAction func addItem(_ sender: Any) {
        // update qty of item
        selectedItem.setValue(qtyVal, forKey: "amountAvailable")
        saveContext()
